...
function getSubFolders($parent_id) {
	...
	$query = "SELECT f.id, f.name, f.description, f.parent_id, f.chat_id, f.last_update, ($query_new_messages) AS notif_new_messages, ($query_nb_messages) AS nb_messages, ($query_nb_folders) AS folders, ($query_nb_files) AS files FROM folder AS f WHERE f.parent_id = $parent_id";
