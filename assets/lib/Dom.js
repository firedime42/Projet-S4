...
;(function (window, document, undefined) {
    ...
    function _indexOf(array, obj) {
        ...
        while (i < len_array && array[i] !== obj) i++;
        return i == len_array ? -1 : i;
    }
    ...
    /**
     * Checks if the given parameter is visible
     * @param {Node} node
     */
    Dom.isVisible = function (node) {
        if (!Dom.isNode(node)) return false;
        if (!Dom.isElement(node)) return Dom.isVisible(node.parentElement);

        let offset = Dom.offset(node);
        let elements = document.elementsFromPoint(Math.floor(offset.left), Math.floor(offset.top));

        return _indexOf(elements, node) >= 0;
    }
    ...
    /**
     * Toggle html element's assignment to the css class
     * @param {HTMLElement|String} element
     * @param {String} className
     */
    Dom.toggleClass = function (element, className) {
        if (element === undefined) {
            throw new Error("Dom.toggleClass first parameter cannot be undefined");
        }

        if (_isString(element)) {
            element = Dom.find(element)[0];
        }
        ...
        if (Dom.hasClass(element, className)) Dom.removeClass(element, className);
        else Dom.addClass(element, className);
    };
    ...
    Dom.replace = function(element, html) {
        ...
        if (Dom.isElement(element))
            parentNode.replaceChild(html, element);
        else {
